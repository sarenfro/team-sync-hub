  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: mData } = await supabase
        .from("team_members")
        .select("id, name, email, ical_url, is_active, color_index, meeting_duration")
        .eq("team_id", team.id)
        .order("created_at");
      if (mData) {
        ...
        mData.forEach((m) => {
          ic[m.id] = (m as any).ical_url ?? "";
          nm[m.id] = m.name;
          em[m.id] = (m as any).email ?? "";
          dur[m.id] = (m as any).meeting_duration ?? 30;
        });
        setIcalInputs(ic);
        setNameInputs(nm);
        setEmailInputs(em);
        setDurationInputs(dur);
  ...
  const handleSave = async (member: Member) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        meeting_duration: durationInputs[member.id] ?? 30,
      } as any)
      .eq("id", member.id);
    if (error) {
      ...
      setMembers((prev) =>
        prev.map((m) =>
          m.id === member.id
            ? { ...m, name: nameInputs[member.id] || m.name, email: emailInputs[member.id] || null, ical_url: icalInputs[member.id] || null, meeting_duration: durationInputs[member.id] ?? 30 }
            : m,
        ),
      );
        ...
        <div className="space-y-4">
          {members.map((member) => (
            <div key={member.id} className="rounded-xl border border-border p-5 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <label className="text-sm text-muted-foreground whitespace-nowrap">Meeting duration</label>
                <select
                  value={durationInputs[member.id] ?? 30}
                  onChange={(e) =>
                    setDurationInputs((prev) => ({ ...prev, [member.id]: parseInt(e.target.value) }))
                  }
